...
The n8n canvas was reviewed end to end: a single `Schedule Trigger` branches into 3 parallel paths — a WAZUH branch, a SPLUNK branch, and a third branch that starts with a shared Cloudflare `HTTP Request` (POST) fetch. The WAZUH and SPLUNK branches each end in their own `Code in JavaScript` → `HTTP Request` (Cloudflare `PUT`) pair; the third branch's shared fetch feeds a `Code in JavaScript2` transform node. This node's output splits in two, but not symmetrically: one branch feeds `HTTP Request2` (`PUT`) directly, while the other passes through a separate `Code in JavaScript3` transform step before feeding `HTTP Request5` (`PUT`) — 4 PUT-ending nodes in total, not one per branch.
...
The 3 parallel branches on the canvas don't write a single shared dataset to Cloudflare KV — they write different payloads. The `HTTP Request1` node in the WAZUH branch sends a `{updated_at, alerts}`-shaped payload (likely the key feeding the "Wazuh Son Alarmlar" panel on the dashboard) — this is the only body shape actually verified via screenshot (`06-cloudflare-kv-put-config.png`). The body content of the other three PUT nodes (`HTTP Request2`, `HTTP Request3`, `HTTP Request5`) was not verified with a screenshot in this audit; that the SPLUNK and third-branch nodes feed the "Threat Live Map" and country-level data is a reasonable inference from the dashboard panels, but the exact payload shape (including field names) has not been confirmed. The **4** separate `HTTP Request` + `PUT` nodes visible on the canvas (`HTTP Request1`, `HTTP Request2`, `HTTP Request3`, `HTTP Request5` — plus one unnamed, shared `HTTP Request` (POST) node that fetches data upstream for two of those rather than being a pair itself) are evidence of this parallel, multi-key update architecture — the workflow doesn't appear to produce a single "alert list," but rather several independent datasets feeding different panels of the dashboard.
